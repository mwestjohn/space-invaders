    ...
    ui(new Ui::MainWindow)
{
    ...
    for(int i = 0 ; i < 55 ; i++) {
        enemy_arr[i] = new enemy();
        //enemy_arr[i]->setPixmap(":/sprites/goomba-transparent-pixel-1.png");
    }
    ...
    if(gameStart==1){
        ...
        for (int i = 0; i < 5;i++) {
            for (int j = 0; j < 12;j++){
                ...
                painter.drawPixmap(x,y,50,50,*(enemy_arr[i]->getSprite()));
                x+=35;
            }
            x=50;
            y+=35;
